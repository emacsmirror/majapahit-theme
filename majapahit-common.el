...
(defun create-majapahit-theme (variant theme-name)
  (let ((class '((class color) (min-colors 257))) ;;
        ...
        (bg1           (if (eq variant 'dark)   "#241F1B" "#FDF9EA" ))
        (bg2           (if (eq variant 'dark)   "#3C352F" "#F7F3E5" ))
        (bg3           (if (eq variant 'dark)   "#4D433A" "#E0DDCE" ))
        (bg4           (if (eq variant 'dark)   "#756456" "#ADA790" ))
...
;;;;; company
     ...
     `(company-tooltip ((,class (:background ,bg2 :foreground ,base :bold t))))
...
;;;;; mode-line
     `(mode-line           ((,class (:foreground ,bg4 :background ,bg2 :box (:color ,m-line-brdr :line-width 1)))))
